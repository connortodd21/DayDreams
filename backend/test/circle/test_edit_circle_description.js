...
describe('Test Edit Circle description', () => {
    ...
    describe('Edit circle without circle description', () => {
        it('Should return 400', (done) => {
            User.findOne({ username: uname }).then((user) => {
                ...
                Circle.findOne({ circleName: 'RAnDoM CirCLE NamE' }).then((circ) => {
                    ...
                    chai.request(server)
                        .post('/circle/edit-circle-description')
                        .set('content-type', 'application/x-www-form-urlencoded')
                        .set('token', token)
                        .send(info)
                        .end((err, res) => {
        ...
        it('Should return 401', (done) => {
            User.findOne({ username: uname }).then((user) => {
                ...
                Circle.findOne({ circleName: 'RAnDoM CirCLE NamE' }).then((circ) => {
                    var info = {
                        circleID: circ._id.toString(),
                        circleDescription: 'Random description'
                    }
                    chai.request(server)
                        .post('/circle/edit-circle-description')
                        .set('content-type', 'application/x-www-form-urlencoded')
