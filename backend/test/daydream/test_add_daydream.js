...
describe('Test add Daydream', () => {
    ...
    after((done) => {
        User.deleteOne({ username: uname }).then(() => {
            Circle.deleteOne({ circleName: 'RAnDoM CirCLE NamE' }).then(() => {
                DayDream.deleteOne({description: 'May 2020, the PurdUe Class of 2020 in 2020,2020'}).then(() => {
                    done()
                })
            })
        })
    ...
    describe('Add daydream without circle id', () => {
            ...
            User.findOne({ username: uname }).then((user) => {
                ...
                Circle.findOne({ circleName: 'RAnDoM CirCLE NamE' }).then((circ) => {
                    var info = {
                        ...
                        description: 'May 2020, the PurdUe Class of 2020 in 2020,2020',
                        totalCost: '4'
                    }
                    chai.request(server)
                        .post('/daydream/add')
    ...
    describe('Add daydream without destination', () => {
        it('Should return 400', (done) => {
            User.findOne({ username: uname }).then((user) => {
                var token = user['tokens'][0]['token'][0]
                Circle.findOne({ circleName: 'RAnDoM CirCLE NamE' }).then((circ) => {
    ...
    describe('Add daydream without description', () => {
        it('Should return 400', (done) => {
            User.findOne({ username: uname }).then((user) => {
                var token = user['tokens'][0]['token'][0]
                Circle.findOne({ circleName: 'RAnDoM CirCLE NamE' }).then((circ) => {
                    var info = {
                        circleID: circ._id.toString(),
                        destination: 'May 2020, the PurdUe Class of 2020 in 2020,2020',
    ...
    describe('Add daydream without total cost', () => {
        it('Should return 400', (done) => {
            User.findOne({ username: uname }).then((user) => {
                var token = user['tokens'][0]['token'][0]
                Circle.findOne({ circleName: 'RAnDoM CirCLE NamE' }).then((circ) => {
